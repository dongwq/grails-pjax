class PjaxGrailsPlugin {
    def version = "0.0.8"
    def grailsVersion = "2.0 > *"

    def title = "Grails Pjax Plugin"
    def author = "John Dong"
    def authorEmail = "dongwqs@sina.com"
    def description = 'Grails Pjax Plugin: more easy pjax in Grails.See more https://github.com/dongwq/grails-pjax'

    def documentation = "https://github.com/dongwq/grails-pjax"

    def license = "APACHE"
    def issueManagement = [system: "GitHub", url: 'https://github.com/dongwq/grails-pjax/issues']
    def scm = [url: 'https://github.com/dongwq/grails-pjax']
}
